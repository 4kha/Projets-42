...
/*   Updated: 2017/12/20 23:06:07 by khtran           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

int		mlx_fractale_julia_4(t_env env, t_value value)
{
	double		tmp;
	double		iteration;
...
void	mlx_fractale_julia_3(t_env env, int x_start, t_lim envmore)
{
	...
	while (x < x_start + 250)
	{
		...
		while (y < 800)
		{
			...
			value.z = (double)(env.pos.x - 500) / 250;
			value.w = (double)(env.pos.y - 400) / 200;
			iteration = mlx_fractale_julia_4(env, value);
